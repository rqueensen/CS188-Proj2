...
class CornersProblem(search.SearchProblem):
    ...
    def getResult(self, state, action):
        ...
        if (not self.walls[nextx][nexty]) and (not self.walls[nextx][nexty]):
            nextPos = (nextx, nexty)
            newVisited = list(state[1])
            if nextPos in self.corners and nextPos not in newVisited:
                newVisited.append(nextPos)
                
            return [nextPos, newVisited]
        else:
            warnings.warn("Warning: checking the result of an invalid state, action pair.")
            return state
...
def cornersHeuristic(state, problem):
    ...
    if state[0] in corners and state[0] not in state[1]:
        curr = 1
    if len(state[1]) + curr == 4:
        return 0
    
    unvisitedCorners = list(corners)
    for visitedCorner in state[1]:
        unvisitedCorners.remove(visitedCorner)
    
    dist = 0
    xy1 = state[0]
    for corner in unvisitedCorners:
        prob = PositionSearchProblem(problem.initGameState, lambda x: 1, corner, xy1, False, False)
        path = search.aStarSearch(prob, manhattanHeuristic)
        if len(path) > dist:
            dist = len(path)
    return dist
...
def foodHeuristic(state, problem):
    ...
    numPellets = len(foodGrid.asList())
    walls = problem.walls
    ...
    for pellet in foodGrid.asList():
        x2, y2 = pellet
        prob = PositionSearchProblem(problem.startingGameState, lambda x: 1, pellet, position, False, False)
        mazeDist = len(search.aStarSearch(prob, manhattanHeuristic))
        problem.heuristicInfo[pellet] = mazeDist
        if mazeDist > dist:
            dist = mazeDist
